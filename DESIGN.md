...
mesmo dia, os Vencidos (`templates/vencidos.html`) a terceira, em 24/09, as Duplicidades (`templates/duplicidades.html`) a quarta o Dashboard (`templates/dashboard.html`, CSS próprio em `aguia-dashboard.css`) a quinta e o Acompanhamento (`templates/colaborador_certificados.html`, CSS em `aguia-acompanhamento.css`; a aba "Escolher certificados" ainda sem redesenho) a sexta e as Carteiras (`templates/carteiras.html`, CSS em `aguia-carteiras.css`; o painel do operador selecionado ainda sem redesenho) a sétima o Instalador (`templates/instalador.html`, CSS em `aguia-instalador.css`) a oitava, os Usuários (`templates/usuarios.html`, CSS em `aguia-usuarios.css`; Departamentos e Níveis de acesso sem redesenho) a nona e a Configuração (`templates/configuracao.html`, CSS em `aguia-config.css`; só a faixa do topo e a aba Alertas por e-mail redesenhadas) a décima, no mesmo dia. Toda tela do portal carrega o Águia; o que resta são abas e painéis que aguardam print, carregando `aguia-tokens.css`,
...
- **Copiar** (`.ag-copiar`, `data-copiar`): botão fantasma pequeno com ícone; nasce oculto e só aparece com `<html class="ag-tem-clipboard">` (área de transferência disponível); o rótulo vira "Copiado" por 1,6s (`cgCopiarInit` em `ui-common.js`, um ouvinte delegado).
- **Carregando** (`.is-carregando` + `aria-busy`): o bloco que já tem conteúdo fica a 55% de opacidade e sem ponteiro; o botão que disparou mostra `.ag-spin` e "Atualizando…" desabilitado; a conclusão vai para a linha de status `aria-live`.
- **KPI-âncora** (`.cg-kpi--link`, da família de listas): `<a>` com a anatomia do KPI, badge escrito no canto (nível de certeza: Confirmado/Provável/Revisar) e o card inteiro como alvo; hover em `gold-500`, foco com o anel. Zero fica no card ("Nenhum encontrado"): a regra de não renderizar contador zerado é do selo, não do resumo.
- **Lista de arquivos** (`.cg-arquivos` / `.cg-arquivo`): caminho relativo à "Pasta base" (mostrada uma vez, no cabeçalho) em mono `text-xs`, quebrando na barra por `<wbr>` com `overflow-wrap: anywhere` de último recurso; caminho completo no `title` e no botão de copiar; vencimento só onde diferencia (mesmo CPF/CNPJ). No terceiro grupo, um `.cg-bloco` por conjunto de nomes parecidos, separado por borda `border-default` e respiro.
- **Barra de proporção** (`.ag-barra`, `.ag-barra__fill`): 6px, pílula, preenchimento em `--grad-gold` porque é proporção e não estado. **Barra empilhada** (`.ag-barra-empilhada__seg--ok|--falha|--neutro`): partes de um todo, sempre com `.ag-legenda` escrita embaixo (cor + número em mono + rótulo).
- **Link com seta** (`.ag-link-seta`): tinta de marca, `text-sm` 600, seta Lucide de 16px que desliza 2px no hover, 44px de altura. É o "para onde ir a partir daqui" no pé de um cartão de resumo; o cartão inteiro nunca é o link, porque há texto para selecionar.
- **Lista rótulo/valor** (`.ag-dados`): linhas com rótulo em `text-secondary` à esquerda e valor em mono tabular à direita, separadas por `border-subtle`; zero fica visível em `text-muted` (`--zero`). Valor que sozinho é problema leva o ícone `triangle-alert` de 14px (`.cg-valor-aviso`), com o rótulo dizendo o que é.
- **Cartão de resumo do Dashboard** (`.cg-dash-card`): cabeçalho com título `text-base` e badge de estado ESCRITO ("Atenção", "Acompanhar", "Em dia") vindo do backend (`estado`); número principal `.cg-numerao` sempre em `text-primary`, Exo 700 `text-4xl` tabular; no máximo seis linhas de lista; pé com nota e link empurrado por `margin-top: auto`; grade `repeat(auto-fill, minmax(min(20rem, 100%), 1fr))` com cartões esticados. Erro fica só no cartão que falhou. Textos pluralizados e rótulos de chave interna vêm prontos do servidor (`app/texto.py`).
- **Abas** (`.ag-abas`, `.ag-abas__aba`): links de verdade (`?aba=…`), 44px, Exo 600; a ativa em texto primário com sublinhado de 2px em `gold-500` e `aria-selected`; a inativa em texto secundário. Com JavaScript viram `tablist`/`tab`/`tabpanel`, setas trocam a aba e a troca é anunciada; a contagem entre parênteses (`__contagem`) em mono.
- **Estado vazio** (`.ag-vazio`): título Exo `text-lg`, explicação `text-sm` secundária e a ação que cria o primeiro, centralizados, até 40rem. É o único lugar em que um botão dourado aparece numa tela sem ação própria (Acompanhamento vazio).
- **KPI neutro no zero** (Acompanhamento): os cartões Vencidos e A vencer só recebem `cg-kpi--perigo`/`cg-kpi--atencao` com valor maior que zero; no zero ficam neutros com "Nenhum". Ativos leva o ícone em tinta de marca (`cg-kpi--marca`). Abaixo de 640px viram três colunas compactas sem ícone nem subtítulo, com o prazo subindo para o rótulo.
- **Modal** (`.ag-modal`, `--lg`): `<dialog>` nativo, `showModal()` prende o foco e Esc fecha; cabeçalho com título Exo e `__x` de 44px, corpo em grade, rodapé com Cancelar em contorno e a ação primária; no celular ocupa a tela inteira. Sem JavaScript o mesmo conteúdo precisaria de página própria (declarado).
- **Zona de arquivo** (`.ag-dropzone--drop`): o `<input type="file">` cobre a zona em opacidade zero (clique, drop e foco vêm dele), tracejado que vira sólido quando há arquivo, `is-over` em dourado, `is-selected` em sucesso; o anel de foco é desenhado na zona.
- **Alerta em bloco** (`.ag-alert--info|success|warning|danger`): mensagem que precisa ficar (o toast some): fundo lavado, borda e texto no matiz do estado, ícone Lucide, título e lista opcional. Erro nunca só no toast.
- **Mestre-detalhe** (Carteiras, `.cg-cart-layout`): `minmax(18rem, 22rem) minmax(0, 1fr)`, lista sticky enquanto couber; cada item é um link `?operador=`, sem rolagem interna, sem carteira primeiro; o selecionado leva o tratamento do item ativo da sidebar (lavado dourado, filete de 3px em `gold-500`, nome em tinta de marca, `aria-current`). Até 1024px vira duas etapas por classe no `<body>` posta pelo servidor: só a lista sem operador, só o detalhe com ele, com `.ag-voltar` (`_voltar.html`) no topo.
- **Voltar** (`.ag-voltar`, partial `_voltar.html`): link com seta Lucide, tinta de marca, 44px; só aparece até 1024px.
- **Abas em trilho** (`.ag-abas--trilho`): as abas rolam na horizontal no estreito, sem barra visível, com a borda direita esmaecida por `mask-image` dizendo que há mais.
- **Campo com sufixo** (`.ag-input-afixo`, `__sufixo`): a unidade dentro do campo ("minutos", "dias"), em `text-muted`; o campo vazio fala pelo placeholder ("Padrão: 5") e a linha "Em vigor: …" (`.ag-ajuda--muted`) diz o que vale agora.
- **Colunas de formulário** (`.ag-form-cols`): `auto-fit` de 18rem, campos alinhados pelo topo; ajuda em `.ag-ajuda`.
- **Etapas** (`.ag-etapas`): indicador de uma cadeia com nomes por extenso (Pedido · Resgate · Conclusão); feita com `check` em `success-700` e texto primário, pendente com `circle` em `text-muted`; o texto completo vai em `sr-only`.
- **Tela administrativa em abas** (Instalador): quatro assuntos, quatro abas por link, cada uma um `.ag-card` com `.cg-inst-cab` (título + badge de situação escrito) e `.cg-inst-corpo`; um único "Atualizar" no cabeçalho da página. Diagnóstico com `.ag-dados` e badge "Tudo em ordem"/"Atenção" mais `.ag-alert--danger` dizendo qual item e quantos; ações destrutivas (Desativar custódia, Expurgar log) confirmam em `.ag-modal` com `.ag-btn--danger` e o tamanho do que vai apagar.
- **Botão só-ícone** (`.ag-btn--icon` 42px, `.ag-btn--icon-sm` 32px, 44px no toque): sempre com `aria-label`. **Menu de ações** (`.ag-menu`, `<details>` nativo): um botão de reticências por linha abrindo Editar, Redefinir senha, Desativar/Reativar; item de perigo em `danger-700`.
- **Interruptor** (`.ag-switch`): checkbox com trilho de 44×24px, `gold-600` ligado, e o estado ESCRITO ao lado num badge ("Ligado"/"Desligado") mais a frase do que acontece.
- **Subtítulo de formulário** (`.ag-form-subhead`): título Exo `text-base` e linha fina, com `space-8` acima; substitui caixa dentro de caixa. Erro de validação: `.is-error` no campo e `.ag-error` embaixo.
- **Barra de ações grudada** (`.ag-sticky-acoes`): `position: sticky; bottom: 0` no pé do cartão, fundo da superfície e fio em cima; "Salvar alterações" em ouro e "Alterações não salvas" em `text-muted` à esquerda quando algo mudou; aviso ao sair com pendência.
- **Papel não é estado** (Usuários): Operador/Gestor em `.ag-badge--neutral`, Administrador em `--gold`; nunca vermelho ou verde para categoria. Status em coluna própria (Ativo/Inativo) e a linha inativa em `text-muted`. Nome é link para editar; e-mail em `.ag-cell-sub`.
