...
/* ══════════════════════════════════════════════════════════════════════════
   Paginação em pílulas (Águia DS)

   Nasceu no Vencidos (24/09) e passou a servir Início e Histórico no mesmo
   dia, porque as três telas mostravam dois modelos de página: o controle
   segmentado legado ("Seguinte") e as pílulas do DS ("Próxima", "…" e a
   última página; "1 de N" no celular, por CSS). Um modelo só, num lugar só.

   `opts`: { nav, ul, pg: {pagina,total_paginas}, goToPage(n), href?(n) }.
   Com `href` os itens são <a> com a query completa (funcionam sem JS);
   sem `href`, são <button> — o estado da tela não vive na URL.
   ══════════════════════════════════════════════════════════════════════════ */

function cgPaginasVisiveis(atual, total) {
  if (total <= 7) return Array.from({ length: total }, (_, i) => i + 1);
  const s = new Set([1, total, atual - 1, atual, atual + 1]);
  if (atual <= 3) [2, 3].forEach((n) => s.add(n));
  if (atual >= total - 2) [total - 1, total - 2].forEach((n) => s.add(n));
  return [...s].filter((n) => n >= 1 && n <= total).sort((a, b) => a - b);
}

function cgPaginacaoPilulas(opts) {
  const nav = _cgResolveEl(opts.nav);
  const ul = _cgResolveEl(opts.ul);
  if (!nav || !ul) return;
  ul.innerHTML = "";
  const pg = opts.pg;
  if (!pg || !pg.total_paginas || Number(pg.total_paginas) <= 1) {
    nav.hidden = true;
    return;
  }
  nav.hidden = false;
  const total = Math.max(1, Number(pg.total_paginas) || 1);
  const atual = Math.min(Math.max(1, Number(pg.pagina) || 1), total);
  const go = typeof opts.goToPage === "function" ? opts.goToPage : null;
  const href = typeof opts.href === "function" ? opts.href : null;

  const item = (n, rotulo, extra) => {
    const el = document.createElement(href ? "a" : "button");
    if (!href) el.type = "button";
    else el.href = href(n);
    el.className = "cg-pag__link";
    el.textContent = rotulo;
    Object.entries(extra || {}).forEach(([k, v]) => el.setAttribute(k, v));
    if (!href && extra && extra["aria-disabled"] === "true") el.disabled = true;
    el.addEventListener("click", (ev) => {
      if (href) ev.preventDefault();
      if (el.getAttribute("aria-disabled") === "true" || el.hasAttribute("aria-current")) return;
      if (go) go(n);
    });
    return el;
  };
  const li = (filho, attrs) => {
  ...
  ul.appendChild(li(item(Math.max(1, atual - 1), "‹ Anterior", atual <= 1 ? { "aria-disabled": "true", tabindex: "-1" } : {})));

  const pos = document.createElement("li");
  pos.className = "cg-pag__posicao";
  pos.textContent = `${atual} de ${total}`;
  ul.appendChild(pos);

  let anterior = 0;
  cgPaginasVisiveis(atual, total).forEach((n) => {
    if (n - anterior > 1) {
      const r = document.createElement("span");
      r.className = "cg-pag__reticencias";
      r.setAttribute("aria-hidden", "true");
      r.textContent = "…";
      ul.appendChild(li(r, { "data-reticencias": "1" }));
    }
    ul.appendChild(li(item(n, String(n), n === atual ? { "aria-current": "page" } : {}), { "data-numero": String(n) }));
    anterior = n;
  });

  ul.appendChild(li(item(Math.min(total, atual + 1), "Próxima ›", atual >= total ? { "aria-disabled": "true", tabindex: "-1" } : {})));
}
